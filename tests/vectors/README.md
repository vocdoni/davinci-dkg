...
| file               | covers                                                          |
|--------------------|-----------------------------------------------------------------|
| `protocol.json`    | `AppMode` + `Role` enum encodings, transcript-domain digests    |
| `derivation.json`  | `S = keccak256(eid ‖ PK_ep ‖ aid) % L` for the per-app key      |
| `schnorr.json`     | operator + organizer Schnorr proof-of-knowledge                 |
| `dleq.json`        | Chaum-Pedersen DLEQ challenges + responses (committee + organizer roles) |
...
- **SDK** — `sdk/tests/vectors.test.ts` reads every file and asserts the
  TypeScript verifiers + `computeS` reproduce each value.
- **Solidity** — `solidity/test/DKGProtocol.t.sol` mirrors the digests and
  enum values inline; SDK vector tests fail if Solidity drifts because both
  layers re-derive `c` over the same Poseidon transcript.
