...
startup and streams the artifact hash checks instead: 0.17 GB measured on node1), and average
0.05 vCPU; the first v5 contribution at `n = 6` cost 550,413
...
**Hardware guidance (on-demand proving keys):** the node loads the contribution and finalize
proving keys for one proof and drops them right after (the decoded contribution key alone is
4.0 GB of heap; the process falls back to 0.26 GB once it is released), so it sits at about 3 GB
at rest (0.3 GB right after start, 2.4–3.0 GB measured on the local testnet after its proofs and
decryption work; v0.5.0 kept all four circuits resident, idled at 8.5–9.6 GB and peaked at
11.2 GB on the Sepolia seed containers) and peaks at about 7 GB during the contribution proof
(cgroup `memory.peak`, local testnet). State the requirement as **12 GB of RAM minimum, 16 GB
comfortable**. On usage-billed hosting memory is the whole bill: it is billed by the GB-hour
while CPU is negligible (0.005–0.014 vCPU average per Sepolia seed node).
