...
// ccommon.CertifyPublicInputs on the compiled partialdecrypt circuit; see the
// helper for why weak simulation-extractability needs it.
func TestPublicInputsHaveDedicatedLeftRows(t *testing.T) {
	c := qt.New(t)
	ccs, err := frontend.Compile(ecc.BN254.ScalarField(), r1cs.NewBuilder, &PartialDecryptCircuit{})
