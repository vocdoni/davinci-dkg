...
func TestDecryptCombineRejectsNonCanonicalLagrangeVector(t *testing.T) {
	...
	deltaOrgPoint := mustDecode(organizerShare(group.Encode(c1)))
...
// With more shares than the threshold the first t Vandermonde equations
// leave an (s − t)-dimensional family of λ vectors that all satisfy them,
// so the system must run to ShareCount for the witness to be unique. Here a
// 2-of-3 combine: μ = (1, −2, 1) is orthogonal to the rows j = 0, 1 (and
// to any degree-1 share polynomial, so C2 still checks out); only the j = 2
// row, Σ μ_k x_k² = 2, rejects λ + μ.
func TestDecryptCombineRejectsNonCanonicalLagrangeVectorAboveThreshold(t *testing.T) {
	c := qt.New(t)
	q := group.ScalarField()
	mul := func(a, b *big.Int) *big.Int { return new(big.Int).Mod(new(big.Int).Mul(a, b), q) }

	// f(x) = 8 + 6x ⇒ d = 14, 20, 26 at x = 1, 2, 3; f(0) = 8.
	r := big.NewInt(5)
	d := []*big.Int{big.NewInt(14), big.NewInt(20), big.NewInt(26)}
	m := big.NewInt(3)
	c1 := group.NewPoint()
	c1.ScalarBaseMult(r)
	deltas := make([]types.CurvePoint, len(d))
	for k := range d {
		p := group.NewPoint()
		p.ScalarBaseMult(mul(d[k], r))
		deltas[k] = group.Encode(p)
	...
	honest, publicInputs, err := BuildWitness(Assignment{
		RoundHash:          new(big.Int).Set(testRoundHash),
		Aid:                new(big.Int).Set(testAid),
		CtIdx:              new(big.Int).Set(testCtIdx),
		OrganizerPK:        organizerPK(),
	...
	c.Assert(err, qt.IsNil)
	c.Assert(publicInputs.ShareCount.Int64(), qt.Equals, int64(3))
	c.Assert(publicInputs.Threshold.Int64(), qt.Equals, int64(2))
	c.Assert(test.IsSolved(&DecryptCombineCircuit{}, honest, ecc.BN254.ScalarField()), qt.IsNil)

	forged := *honest
	shift := []int64{1, -2, 1}
	for k, mu := range shift {
		lambda, ok := honest.LagrangeCoefficients[k].(*big.Int)
		c.Assert(ok, qt.IsTrue)
		forged.LagrangeCoefficients[k] = new(big.Int).Mod(new(big.Int).Add(lambda, big.NewInt(mu)), q)
	}
	assert := test.NewAssert(t)
	assert.SolvingFailed(&DecryptCombineCircuit{}, &forged, test.WithCurves(ecc.BN254))
}
