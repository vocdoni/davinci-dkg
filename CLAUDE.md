...
- **BRLC transcript encoding** (contribution calldata): `circuits/common/brlc.go`, `web3/brlc.go` and
  `solidity/src/libraries/BRLC.sol` must agree bit-for-bit.
...
logical storage. Phase windows are constructor immutables (`EPOCH_DURATION_BLOCKS`,
`COMMITTEE_SELECTION_BLOCKS`, `KEY_ASSEMBLY_BLOCKS`, `FINALIZE_GAP_BLOCKS`). `interfaces/*.sol` is the
