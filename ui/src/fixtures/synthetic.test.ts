...
describe('synthetic fixture', () => {
  ...
  it('delivers partials in waves of t, withholds some shares, combines most', () => {
    ...
    for (const key of Object.keys(store.ciphertexts)) {
      ...
      if (!ct.organizerShare) withheld += 1
      if (ct.combined) combined += 1
    }
    expect(withheld).toBe(24)
    expect(combined).toBe(68)
    // A few shares were re-submitted before the combine.
    expect(
      Object.values(store.ciphertexts).filter((ct) => (ct.organizerShare?.overwrites ?? 0) > 0).length,
    ).toBeGreaterThan(0)
  })
