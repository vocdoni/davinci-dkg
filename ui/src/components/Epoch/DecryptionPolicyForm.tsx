...
const fields: { key: keyof Omit<DecryptionPolicyFormState, 'ownerOnly'>; label: string; help: string }[] = [
  { key: 'maxDecryptions', label: 'Max ciphertexts', help: 'Per epoch; blank = unlimited.' },
  ...
  return (
    <Stack gap={4}>
      <Field.Root disabled={disabled} display='flex' alignItems='center' gap={3}>
        ...
        <Text fontSize='sm'>Owner-only — only the epoch organizer may submit ciphertexts</Text>
      </Field.Root>
