...
import { phaseSequence, roundPhase, roundPhaseLabel, type EpochPhase } from '~lib/epoch-utils'
...
// the segments behind the cursor. Aborted epochs short-circuit to a single
// coral row.
//
// The pulse keyframes live in the global theme (`dkgPhasePulse`) so this
// component doesn't need its own <Global> tag.
export function PhaseTimeline({ epoch }: { epoch: Epoch }) {
  const current = roundPhase(epoch)
